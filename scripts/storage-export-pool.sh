#!/usr/bin/env bash
# Export the current authenticated immutable pool for the next repository build.
...
[ "$#" -eq 4 ] || dkc::die \
	"usage: storage-export-pool.sh <toolbox> <state-result> <connection-or-empty> <output-root>"
toolbox="$1"
state_result="$(realpath "$2")"
provided_connection="$3"
output_root="$4"
podman image exists "$toolbox" || dkc::die "toolbox image is missing; run: make image"

stage="$DKC_RUN_DIR/storage-export-pool"
...
mkdir -p "$output_root"
[ ! -e "$output" ] || dkc::die "refusing to replace live-pool export output"
name="dkc-storage-export-pool-${DKC_RUN_ID}"
...
if ! dkc::archive_worktree |
	podman run --rm --interactive --read-only --read-only-tmpfs=false \
		...
		exec python3 scripts/in-container/storage-export-pool.py \
			--state-result /input/state --connection /run/secrets/storage.json \
			--keyring /work/repo/keys/dkc-archive-keyring.gpg \
			--signing-subkeys /work/repo/keys/archive-signing-subkeys.fingerprints \
			--output "/output/$1"
	' sh "$DKC_RUN_ID"; then
	dkc::die "authenticated live-pool export failed"
fi
(
	cd "$output"
...
dkc::ok "authenticated live-pool export passed"
