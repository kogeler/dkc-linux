...
A flavor policy is the same for every Debian source, so the exceptions that do
depend on the kernel series live in the matching source profile instead; see
[../source-profiles/README.md](../source-profiles/README.md).

The profile's `fpu` section is an exact list of C objects that the kernel itself
compiles with `CC_FLAGS_FPU`. It deliberately contains no directory globs. A new
or removed object therefore fails the command audit and requires review. Its
final-module symbols are derived from those exact objects during the build, then
reconciled against the disassembled module.

The profile's `simd_allowlist` contains the other exact final-artifact/symbol
pairs reviewed from that series' machine code: optimized crypto/RAID
implementations, KVM guest-FPU access, and explicitly bracketed display copies.
The full audit
